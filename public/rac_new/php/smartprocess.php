...
/*	---------------------------------------------------------------------------
	...
	$senderorganisation = strip_tags(trim($_POST["senderorganisation"]));
...
/*	-------------------------------------------------------------------------
	...
	//validate telephone
	if(isset($_POST["sendertelephone"])){
			if (!$sendertelephone) {
				$errors[] = "You must enter a telephone.";
			} elseif(strlen($sendertelephone) < 4)  {
				$errors[] = "Your must enter a valid telephone.";
			}
	}
	
	//validate organisation
	if(isset($_POST["senderorganisation"])){
			if (!$senderorganisation) {
				$errors[] = "You must enter your organisation.";
			} elseif(strlen($senderorganisation) < 2)  {
				$errors[] = "Organisation must be at least 2 characters.";
			}
	}	
	...
	if ($errors) {
		...
		if($recipients == true){
			$recipients = array(
				"admin@ecowebplus.com" => "Admin Ecowebplus",
				"dosso.ndessomin@bwint.org" => "Regional Representative"
			);
		...
		if($mail->Send()) {
			...
			/*	---------------------------------------------------------------------
				...
				if($autoResponder == true){
					...
					$automail->Subject = "Thank you for your message";
